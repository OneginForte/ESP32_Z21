...
//#define S88useT3  //S88 mit Timer3 (if active data lost on loconet!!!)

#elif defined(SANGUINO_MCU) //Sanguino (other pins!)
#define S88noT  //S88 ohne Timer - data lost on loconet!
...
#define TIMER3_Time 0xF788 //1100Hz (old: 440Hz = 0xBA00)
...
void SetupS88() {
  ...
  S88Module = FIXSTORAGE.read(EES88Moduls);
  if (S88Module > S88MAXMODULE) { //S88 on!
    S88Module = S88MAXMODULE;   //S88 max Module
    FIXSTORAGE.FIXMODE(EES88Moduls, S88MAXMODULE); //save correct max Value
  }
