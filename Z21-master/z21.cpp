	...
	switch (header) {
		...
		case LAN_GET_CODE:	//SW Feature-Umfang der Z21   
		  /*#define Z21_NO_LOCK        0x00  // keine Features gesperrt 
			#define z21_START_LOCKED   0x01  // Ñz21 startî: Fahren und Schalten per LAN gesperrt 
			#define z21_START_UNLOCKED 0x02  // Ñz21 startî: alle Feature-Sperren aufgehoben */
		...
		case (LAN_SET_BROADCASTFLAGS): {
			...
			if (notifyz21RailPower)
			  ...
			  // 1=BC Power, Loco INFO, Trnt INFO; 2=BC ƒnderungen der R¸ckmelder am R-Bus
			#endif
			break;
		  }
		...
		case (LAN_GET_LOCOMODE):
			...
			Es kˆnnen maximal 256 verschiedene Lok-Adressen abgelegt werden. Jede Adresse >= 256 ist automatisch DCC.
			*/
			data[0] = packet[4];
			data[1] = packet[5];
		...
		case (LAN_GET_TURNOUTMODE):
			...
			Es kˆnnen maximal 256 verschiedene Funktionsdecoder -Adressen gespeichert werden. Jede Adresse >= 256 ist automatisch DCC.
			*/
			data[0] = packet[4];
			data[1] = packet[5];
			data[2] = 0;	//0=DCC Format; 1=MM Format
		...
		case (LAN_RAILCOM_GETDATA): {
			  uint16_t Adr = 0;
			  if (packet[4] == 0x01) {	//RailCom-Daten f¸r die gegebene Lokadresse anfordern
...
void z21Class::setExtACCInfo(uint16_t Adr, byte State, bool Status) {
	...
	data[2] = Adr & 0xFF; //Low
	data[3] = State;
	data[4] = Status;  //0x00 Ö Data Valid; 0xFF Ö Data Unknown
